...
The `country` parameter is also optional but can be used to preload the Klyme Widget with banks from a specific as opposed to displaying the entire European banks list.

Alternatively, you can simply pass `auto` as the `country` parameter and let the Klyme Widget handle everything — automatically detecting the user’s location and displaying region-specific banks.
...
Call the `/payment-auth-requests` endpoint to create a payment authorisation request and generate a unique `uuid` that you then insert into the `id` parameter of the
Klyme HTML element.
...
`redirectUrl` you will have specified in **Step 2** when making the call to `/payment-auth-requests`.
...
If you wish to receive the complete payload of a payment, you have two options. Either leverage our webhook notifications functionality or you can make
an API call to our `/payments/status` endpoint.
